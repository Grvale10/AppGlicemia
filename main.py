...
def gerar_pdf_detalhado(df_hist, df_pacs):
    ...
    for p_nome in pacientes_no_hist:
        ...
        pdf.cell(25, 10, " Carbos", border=1, fill=True)
        pdf.cell(25, 10, " Dose (U)", border=1, fill=True)
        pdf.cell(25, 10, " Glic. Pos", border=1, fill=True, ln=True)
        ...
        for _, row in dados_p.iterrows():
            ...
            pdf.cell(35, 8, str(row.get('Momento', 'Refeicao')), border=1)
...
if aba == "🏠 Início":
    ...
    with aba_p[0]:
        with st.form("add_pac"):
            ...
            with c2: s = st.selectbox("Tipo Sanguíneo", lista_tipos_sangue); tp_plano = st.selectbox("Plano de Saúde", lista_planos); detalhe_plano = st.text_input("Dados do Plano (Opcional)")
